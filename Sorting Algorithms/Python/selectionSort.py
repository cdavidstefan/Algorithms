...
def selection_sort(arr):
    ...
    for i in range(0, n):
        ...
        for j in range(i + 1, len(arr)):
            ...
            # Daca elementul de pe pozitia j (elementul curent) este mai mic decat elementul de pe pozitia min
            # Inseamna ca am gasit un element mai mic in sirul nesortat si ii salvam indexul in variabila min
            if arr[j] < arr[min]:
                min = j

            # iesim din bucla interioara

        # la fiecare parcurgere a listei, in bucla exterioara punem elementul minim gasit
        # pe pozitia lui i
